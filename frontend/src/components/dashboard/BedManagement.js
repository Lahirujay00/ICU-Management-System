...
export default function BedManagement() {
  ...
  const handleBedAssignment = async (bedId, patientId) => {
    try {
      await apiClient.assignPatientToBed(bedId, patientId)
            ...
            <div className="space-y-2 text-sm">
              {bed.patient ? (
                <div className="space-y-1">
                  ...
                  <p className="text-gray-600">Room: {bed.roomNumber}</p>
              ...
              <div className="flex gap-2">
                {bed.status === 'occupied' && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      handleBedStatusChange(bed._id, 'cleaning')
                    }}
