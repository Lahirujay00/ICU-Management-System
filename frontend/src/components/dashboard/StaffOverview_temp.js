...
const StaffTimetableView = ({ staff }) => {
  ...
  const doctorShifts = {
    morning: { start: 8, end: 16, label: '8AM-4PM' },
    afternoon: { start: 13, end: 19, label: '1PM-7PM' },
    night: { start: 19, end: 7, label: '7PM-7AM' },
    emergency: { start: 0, end: 24, label: '24h On-call' }
  }

  const nurseShifts = {
    morning: { start: 7, end: 13, label: '7AM-1PM' },
    afternoon: { start: 13, end: 19, label: '1PM-7PM' },
    night: { start: 19, end: 7, label: '7PM-7AM' }
  }
  ...
  const getShiftColor = (role, shift) => {
    const colors = {
      doctor: {
      ...
      nurse: {
        ...
        night: 'bg-cyan-100 text-cyan-800 border-cyan-200'
      }
    }
    return colors[role]?.[shift] || 'bg-gray-100 text-gray-800 border-gray-200'
  }
        ...
        <div key={role} className="bg-white rounded-lg shadow-sm border overflow-hidden">
          <div className="bg-gray-50 px-6 py-4 border-b">
            <h4 className="text-lg font-semibold text-gray-900 capitalize">
              {role === 'doctor' ? 'Doctors' : role === 'nurse' ? 'Nurses' : `${role}s`} Timetable
            </h4>
            <p className="text-sm text-gray-600 mt-1">
              {roleStaff.length} staff members • 
              {role === 'doctor' ? 'Shifts: 8AM-4PM, 1PM-7PM, 7PM-7AM, Emergency' : 'Shifts: 7AM-1PM, 1PM-7PM, 7PM-7AM'}
            </p>
          </div>
              ...
              <thead>
                <tr className="bg-gray-50">
                  ...
                  {getWeekDates().map((date, index) => (
                    <th key={index} className="px-3 py-3 text-center text-sm font-medium text-gray-900 min-w-[120px]">
                ...
                {roleStaff.map((member) => (
                  <tr key={member._id} className="hover:bg-gray-50">
                    <td className="px-4 py-4">
                      <div className="flex items-center space-x-3">
                        <div className={`w-3 h-3 rounded-full ${
                          member.isOnDuty ? 'bg-green-400' : 'bg-gray-300'
                        }`}></div>
                        <div>
                    ...
                    {getWeekDates().map((date, dayIndex) => {
                      ...
                      const shifts = role === 'doctor' ? doctorShifts : nurseShifts
                      const dayShifts = Object.keys(shifts)
                      const randomShift = member.currentShift || dayShifts[dayIndex % dayShifts.length]
                        ...
                        <td key={dayIndex} className="px-3 py-4 text-center">
                          {randomShift !== 'off' ? (
                            <div className={`inline-block px-2 py-1 rounded-full text-xs font-medium border ${
                              getShiftColor(role, randomShift)
                            }`}>
                              {shifts[randomShift]?.label || randomShift}
                            </div>
                          ) : (
          ...
          <div className="bg-gray-50 px-6 py-4 border-t">
            <div className="flex items-center space-x-4 text-sm">
              ...
              {Object.entries(role === 'doctor' ? doctorShifts : nurseShifts).map(([shiftKey, shift]) => (
                <div key={shiftKey} className="flex items-center space-x-2">
                  <div className={`w-3 h-3 rounded border ${getShiftColor(role, shiftKey).replace('text-', 'bg-').split(' ')[0]}`}></div>
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-green-600">{staff.filter(s => s.isOnDuty).length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Off Duty:</span>
              <span className="font-medium text-red-600">{staff.filter(s => !s.isOnDuty).length}</span>
            </div>
          </div>
        </div>
